...
window.addEventListener("load", function () {
    // alert("Activate Jecto");
    setTimeout(() => {
        document.querySelector(".preloader").style.display = "none";
    }, 1000);
    speak('Activating Jecto');
    speak('going online');
    wishMe();
});
...
function speakThis(message) {
    ...
    if (message.includes('hi') || message.includes('hello') || message.includes('hii') || message.includes('hey')) {
        ...
        const finalText = 'My name is Jecto, i was created by master priyanshu';
        speech.text = finalText;
    }

    else if (message.includes('who is priyanshu')) {
        const finalText = 'priyanshu is my god who created me he is in class 11 and studiying in kendriya vidyalaya';
        speech.text = finalText;
    }

    else if (message.includes('tell me about my friends')) {
        const finalText = 'Anshu, yash, akshay ';
        speech.text = finalText;
    }

    else if (message.includes('message')) {
        const finalText = 'ok sir the message is sent';
        speech.text = finalText;
    }

    else if (message.includes('exam')) {
        const finalText = 'ok sir please wait 2 minutes i am finding best questions for your exam';
        speech.text = finalText;
    }

    else if (message.includes('thank')) {
        const finalText = 'dont worry sir i am here to help you';
        speech.text = finalText;
    }

    else if (message.includes('alarm')) {
        const finalText = 'ok sir maine alarm laga diya hain';
        speech.text = finalText;
    }

    else if (message.includes('open my instagram')) {
        const finalText = 'Your instagram has been deactivated';
        speech.text = finalText;
    }
    ...
    else if (message.includes('open my school website')) {
        window.open(`https://narela.kvs.ac.in/`, "_blank");
        const finalText = 'opening priyanshu sir school website';
        speech.text = finalText;
    }
